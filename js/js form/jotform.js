...
var JotForm = {
    ...
    encryptAll  : function(e, cb) {
        ...
        $$('.form-textbox, .form-textarea, .form-radio, .form-checkbox, .form-dropdown > option').each(function(field){
            // detect control_ type, we need it as the callback parameter.
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    editMode: function (data, noreset, skipField) {
        ...
        if (!JotForm.debug) {
            if (this.url.search("https") == -1) {
                preLink = "http://max.jotfor.ms/";
            } else {
                preLink = "https://static-interlogyllc.netdna-ssl.com/";
            }
        }
     ...
     * @param {Object} field
     */
    hideField: function (field, multiple) {
        ...
        if (condition.type == 'field') { // Field Condition
            ...
            condition.action.each(function (action) {
                ...
                if ($('input_' + action.field) && $('input_' + action.field).triggerEvent) {
                    if (!condition.terms.any(function (term) {
                            return term.field == action.field;
                        })) {  //rule matches action so don't rerun to avoid infinite loop
    ...
    calculationType: function (id) {
        ...
        } else if ($('id_' + id).readAttribute('data-type') == 'control_matrix') {
            return 'matrix';
        } else {
    ...
    setCurrencyFormat: function (curr, useDecimal) {
    ...
    countTotal: function (prices) {
            ...
            if (!pair.value.recurring) {
                ...
                if (priceText) {
                    ...
                    if (price == "0" && pair.value.specialPriceList && defaultSpecial) {
                        $(priceText).update(parseFloat(defaultSpecial || 0).formatMoney(decimal));
        ...
        if ($("payment_total")) {
            ...
            if ($("payment_total").up(".form-line") && $("payment_total").up(".form-line").triggerEvent) {
                $("payment_total").up(".form-line").triggerEvent("click");  //b#520074 trigger calculation
            }
        }
    },
    prices: {},
                    ...
                    if (jQuery(pad).parent().parent().parent().is(":visible")) {
                        ...
                        if (jQuery(pad).jSignature('getData', 'base30')[1].length == 0 && !jQuery(pad).hasClass('edit-signature')) {
                            ...
                            if (w.find(".form-line-error").length == 0) {
                                var preLink = ( JotForm.url.search("https") == -1 ) ? "http://cdn.jotfor.ms/" : "https://static-interlogyllc.netdna-ssl.com/";
            ...
            form.observe('submit', function (e) {
                try {
                    ...
                    if(JotForm.isEncrypted) {
                        JotForm.encryptAll(e, function( type ){
    ...
    handlePreview: function(filled) {
        $A(JotForm.forms).each(function (form) {
            ...
            if (filled === true) {
                ...
                script.setAttribute('src', 'https://www.jotform.com/js/form-tester.js');
